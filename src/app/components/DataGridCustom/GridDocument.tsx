...
const EditToolbar = (props: any) => {
  ...
  const handleClick = () => {
    ...
    setRows((oldRows: any) => [...oldRows, { id, name: '', type: '', archive: null, isNew: true }])
...
export default function GridDocument({ data, actionUpsert, actionDelete }: any) {
  ...
  const FileEditCell = (props: GridRenderEditCellParams): React.ReactNode => {
        const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const file = event.target.files?.[0];
        if (file) {
            props.api.setEditCellValue({ id: props.id, field: props.field, value: file }, event);
        }
    };

    return (
        <Box sx={{position:  'relative', borderRadius: '100px',  width: 'fit-content', padding: '3px'}}>
            <AttachFile sx={{color: '#828DD4', cursor: 'pointer'}} />
            <input style={{position: 'absolute', left: 0, width: '100%', height: '100%', cursor: 'pointer', opacity: 0}} type="file" onChange={handleFileChange} />
        </Box> 
    );
  }
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
        ...
        editable: true,
        renderCell: (params) => (
            params.value ? <span>{(params.value as File).name}</span> : <i>Nenhum arquivo</i>
          ),
        renderEditCell: (params) => <FileEditCell {...params} />,
      },
    {
      field: 'actions',
      type: 'actions',
