...
const useFetchPowerBiReports = (activeCompany, workspaceId) =>
  useCallback(async () => {
    if (!activeCompany || !workspaceId) return []
    ...
    try {
      const response = await fetch(`/api/getPowerBiReportsByGroup?groupId=${workspaceId}`, {
        headers: {
          'Client-Id': clientId,
...
export default useFetchPowerBiReports
