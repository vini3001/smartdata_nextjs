...
export type filterUser = typeof filterObject

interface UserFiltersProps {
  handleSetValue: (value: filterUser) => void
}

export default function UserFilters({handleSetValue}: UserFiltersProps) {
  const [isOpen, setIsOpen] = React.useState<boolean>(false);
  const [local, setLocal] = React.useState(filterObject)

  const userFilters = filters.people
      ...
      >
        ...
        >
          ...
          <div style={{display: 'flex', flexDirection: 'column', marginTop: '1rem', gap: '0.1rem'}}>
            {userFilters.length !== 0 && userFilters.map((item) => {
              return (
