...
export async function getReportsByGroupIdInfo(headerValues, params) {
  ...
  try {
    const groups = await getReportsByGroupId(
      params.groupId,
      clientId,
      authority,
      tenantId,
...
async function getReportsByGroupId(
  ...
  const reportGroupsApi = `https://api.powerbi.com/v1.0/myorg/groups/${groupId}/reports`;
  ...
  const result = await fetch(reportGroupsApi, {
    method: "GET",
    headers,
  });
